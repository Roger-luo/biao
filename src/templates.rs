...
impl TemplateManager {
    ...
    pub fn list(&self) -> Result<Vec<TemplateInfo>> {
        ...
        for dir in &self.template_dirs {
            ...
            Self::collect_templates_in_dir(dir, dir, &mut map);
        }
    ...
    pub fn get(&self, name: &str) -> Result<String> {
        ...
        for dir in &self.template_dirs {
            ...
            let path = dir.join(candidate).with_extension("toml");
            if path.exists() {
    ...
    fn builtin_templates() -> Vec<(&'static str, &'static str)> {
        vec![
            ...
            ("rusty/priority", "Rust-style priority labels (P-critical, P-high, etc.)"),
            ("type", "Type-based labels (type/bug, type/feature, type/docs, etc.)"),
            ("type/labels", "Type-based labels (type/bug, type/feature, type/docs, etc.)"),
            ("area", "Area-based labels (area/api, area/cli, area/docs, etc.)"),
            ("rusty/area", "Area-based labels (A-api, A-cli, A-docs, etc.)"),
    ...
    fn get_builtin_template(name: &str) -> Option<String> {
        match name {
            ...
            "rusty/priority" => Some(TEMPLATE_PRIORITY_PREFIXED.to_string()),
            "type" => Some(TEMPLATE_TYPE.to_string()),
            "type/labels" => Some(TEMPLATE_TYPE.to_string()),
            "area" => Some(TEMPLATE_AREA.to_string()),
            "rusty/area" => Some(TEMPLATE_AREA.to_string()),
    ...
    fn collect_templates_in_dir(
        base_dir: &Path,
        dir: &Path,
        map: &mut std::collections::HashMap<String, TemplateInfo>,
    ) {
        if let Ok(entries) = fs::read_dir(dir) {
            for entry in entries.flatten() {
                let path = entry.path();
                if path.is_dir() {
                    Self::collect_templates_in_dir(base_dir, &path, map);
                    continue;
                }
                if path.extension().map_or(false, |ext| ext == "toml") {
                    ...
                    let rel_without_ext = rel.with_extension("");
                    let key = rel_without_ext
                        .to_string_lossy()
                        .replace('\\', "/");
...
mod tests {
    ...
    fn test_template_list_reads_description() {
        ...
        let nested_dir = temp_dir.join("rusty");
        fs::create_dir_all(&nested_dir).unwrap();

        let template_path = nested_dir.join("custom.toml");
